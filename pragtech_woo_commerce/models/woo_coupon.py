...
class WooCoupon(models.Model):
    ...
    discount_type = fields.Selection([('percentage', 'Percentage discount'), ('fixed_amount', 'Fixed cart discount'),
    ...
    discount_specific_product_ids = fields.Many2many('product.product', 'product_coupon_rel', 'product_id',
                                                     'coupon_program_id', string="Discount Products")
                ...
                if parsed_data:
                    for rec in parsed_data:
                        ...
                        for product in products:
                            odoo_product = self.env['product.product'].sudo().search([('woo_id', '=', product)],
                                                                                     limit=1)
                        ...
                        if discount_type and discount_type == 'fixed_product':
                            ...
                            vals.update({
                                'discount_specific_product_ids': [(4, product.id, False) for product in product_list
                                                                  if product]
                            })

                        coupon = self.env['coupon.program'].sudo().search([('woo_id', '=', rec.get('id'))], limit=1)
                        if not coupon:
    ...
    def export_selected_coupon(self, instance_id):
        ...
        selected_records = self.env['coupon.program'].sudo().browse(selected_ids)

        if selected_records:
            records = selected_records
        else:
            records = self.env['coupon.program'].sudo().search([('is_exported', '=', False)])
